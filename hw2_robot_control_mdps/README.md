...
> Keypoints fall outside the robot's workspace, causing IK to fail to converge (no valid joint solution exists).

1. What can happen if you change the dt parameter in IK?

> A larger dt (equivalent with $\alpha$ or step size from lecture) speeds up convergence but risks overshooting; a smaller dt is more stable and accurate but converges slower. Similar to learning rate in deep learning gradient descent.
...
> Numerical IK is general-purpose (works for any robot kinematics) but slower and may get stuck in local minima; analytical IK is fast and exact but robot-specific and hard to derive for manipulators with many DoFs.

1. What are the limits of our IK solver compared to state-of-the-art IK solvers?

> Our solver only tracks position (ignores orientation), uses basic DLS without joint limits/obstacle/collision constraints, lacks multi-task prioritization, and provides non global optimality guarantees.
...
> Overcorrection, osciliation as part of the violent error recovery.

1. How does $K_D$ mitigate the effect you saw above when increasing $K_P$?

> Adding $K_D$ act as a damper that stops the arm from overshooting.

1. In what scenarios is a non-zero $K_I$ needed for the controller to perform well?

> A non-zero integral gain $K_I$ in a PID controller is necessary when a system needs to eliminate steady-state error and handle unmeasured external forces.
...
> RL policy performance over 3 runs (Average final EE tracking error): [0.0145, 0.0068, 0.0056] -> mean 0.00897, std 0.00394
...
> The RL policy doesn't track the keypoints as closely as the PID controller.
