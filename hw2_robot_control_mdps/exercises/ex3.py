...
def process_action(action: np.ndarray, jnt_range: np.ndarray) -> np.ndarray:
    ...
    action_norm = (action - action_lo) / (action_hi - action_lo)
    return jnt_range[:, 0] + action_norm * (jnt_range[:, 1] - jnt_range[:, 0])
...
def get_obs(
    ...
    """
    # have: T^w_ee, r^w_ee: world -> ee
    # have: T^w_base, r^w_base: world -> base
    # have: T^w_target: world -> target
    # want: T^base_ee, r^base_ee
    # ee_pos_base
    # ee_rot_base -rot_mat_to_quat(), quat_normalize()-> ee_quat_base
    # target_pos_base
    w_rot_base = base_rot_w.T  # (3, 3)
    ee_pos_base = w_rot_base @ (ee_pos_w - base_pos_w)  # (3,)

    ee_rot_base = w_rot_base @ ee_rot_w  # (3, 3)
    ee_quat_base = quat_normalize(rot_mat_to_quat(ee_rot_base))  # (4,)

    # NOTE: equivalently in quaternion
    # base_quat_w = rot_mat_to_quat(base_rot_w)
    # w_quat_base = quat_conjugate(base_quat_w)
    # ee_quat_w = rot_mat_to_quat(ee_rot_w)
    # ee_quat_base = quat_normalize(quat_mul(w_quat_base, ee_quat_w))

    target_pos_base = w_rot_base @ (target_pos_w - base_pos_w)  # (3,)
    return np.concat(
        [
            qpos,  # (num_joints,)
            ee_pos_base,  # (3,)
